...
$redirect = str_replace( '/user/login', '/', $redirect );
if( $redirect && !$url )
{
    $http->setSessionVariable( 'CrossRedirect', 'http://' . $redirect );
}
elseif ( $redirect && $url )
{
    $http->setSessionVariable( 'CrossRedirect', 'http://' . $redirect . '/' . $url );
}
elseif ( !$redirect && $url )
{
    $http->setSessionVariable( 'CrossRedirect', 'http://' . $_SERVER['HTTP_HOST'] . '/' . $url );    
}

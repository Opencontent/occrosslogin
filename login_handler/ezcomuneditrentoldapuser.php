<?php
//
// Definition of eZComuneDiTrentoLDAPUser class
//


/*!
  \class eZComuneDiTrentoLDAPUser ezcomuneditrentoldapuser.php
  \ingroup eZDatatype
  \brief The class eZComuneDiTrentoLDAPUser does
...
class eZComuneDiTrentoLDAPUser extends eZUser
{
    /*!
     Constructor
    */
    ...
    static function returnFalseIfHasPasswordExpiry( $user )
    {
        if ( $user instanceof eZUser )
        {
            $userID = $user->attribute( 'contentobject_id' );
            if ( eZINI::instance( 'crosslogin.ini' )->hasVariable( 'LoginHandersSettings', 'ExcludeLDAPIfUserPaex' ) &&
                 eZINI::instance( 'crosslogin.ini' )->variable( 'LoginHandersSettings', 'ExcludeLDAPIfUserPaex' ) == 'enabled' )
            {                
                $paexUser = eZPaEx::fetch( $userID );                
                if ( $paexUser instanceof eZPaEx && $paexUser->attribute( 'passwordlifetime' ) == 0 )
                {
                    return $user;
                }
                eZDebug::writeWarning( "$userID e' un utente con scadenza password: l'autenticazione LDAP viene ignorata" );
            }
            else
            {
                return $user;
    ...
    */
    static function loginUser( $login, $password, $authenticationMatch = false )
    {
        ...
        if ( $exists and $isEnabled and $canLogin )
        {
            ...
            return self::returnFalseIfHasPasswordExpiry( $user );
        }
...
// GAB original:             if  ( !@ldap_bind( $ds, $info[0]['dn'], $password ) )
                     ...
                     $LDAPGroupMappingType == $ByMemberAttributeHierarhicaly )
                {
                    ...
                    eZComuneDiTrentoLDAPUser::getUserGroupsTree( $requiredParams, $newfilter, $userData['dn'], $groupsTree, $stack, $groupSearchingDepth );
                ...
                $existingUser = eZComuneDiTrentoLDAPUser::publishUpdateUser( $extraNodeAssignments, $defaultUserPlacement, $userAttributes, $isUtf8Encoding );
                ...
                ldap_close( $ds );
                return self::returnFalseIfHasPasswordExpiry( $existingUser );
            }
            else
            {
                eZDebug::writeError( 'Cannot initialize connection for LDAP server', 'eZComuneDiTrentoLDAPUser::loginUser()' );
                $user = false;
                return $user;
            }
        }
    ...
    */
    ...
    {
        $thisFunctionErrorLabel = 'eZComuneDiTrentoLDAPUser.php, function publishUpdateUser()';
        ...
        if ( $createNewUser )
        {
            ...
            $user = eZComuneDiTrentoLDAPUser::create( $userID );
...
if ( self::returnFalseIfHasPasswordExpiry( $user ) == false )
{
    return $user;
}
                    ...
                    if ( $isAssignmentExist )
                    {
                        ...
                        if ( !is_object( $existingNode ) )
                        {
                            eZDebug::writeError( "Cannot find assigned node as $defaultUserPlacement's child.",
                                                 'kernel/classes/datatypes/ezuser/ezcomuneditrentoldapuser' );
                        }
                        else
                        {
    ...
    */
    ...
    {
        $thisFunctionErrorLabel = 'eZComuneDiTrentoLDAPUser.php, function publishNewUserGroup()';
        $newNodeIDs = array();
    ...
    */
    static function goAndPublishGroups( &$requiredParams,
                                 ...
                                 $isUser = false )
    {
        $thisFunctionErrorLabel = 'eZComuneDiTrentoLDAPUser.php, function goAndPublishGroups()';
        ...
        if ( count( $parentNodesForNew ) > 0 )
        {
            ...
            {
                $newNodeIDs = eZComuneDiTrentoLDAPUser::publishNewUserGroup( $parentNodesForNew, array( 'name' => $currentName,
                                                                                          'description' => '' ) );
            }

...
for ( $i = 1; $i < count( $argv ); ++$i )
{
    ...
    {        
             ...
             $arg[1] == '-' )
        {
            ...
            else if ( substr( $flag, 0, 4 ) == 'user' )
            {
                $parse = explode( '=', substr( $flag, 4 ) );                
                $userid = explode( ';', $parse[1] );                
            }
            else
            {
                help();
...
$cli->output( "eZPaEx: Set password expired to current time in all existing users" );
...
if ( $userid )
{
    $where = "WHERE contentobject_id IN ( " . implode( ', ', $userid ) . " )";
}

$query = "UPDATE $table SET password_last_updated = 0 " . $where;
